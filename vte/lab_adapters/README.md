...
## Biological comparability scope

The lab adapter layer is not intended to prove that a model trace visually resembles a biological animal trace. Its role is to translate external behavioral and neural datasets into explicit, auditable comparison endpoints.

For Stage 3.2C, the primary biological target is choice behavior under interpretable task conditions:

- chosen arm / route / path;
- reward, outcome, correctness, or error when recoverable;
- uncertainty, conflict, reversal, perturbation, or carryover condition when recoverable;
- dwell, reorientation, IdPhi-like trajectory measures, or neural representational competition only as secondary readouts around choice episodes.

VTE-like behavior is treated as a secondary behavioral readout, not as the primary claim. The primary model claim remains gate-viscosity dynamics: boundary conditions should produce delayed or unstable commitment, obvious conditions should produce confident commitment, strong one-shot events should update future choice, and carryover should persist after stimulus removal.

Current adapter roles:

- `crcns_wtrack`: exploratory W-track ingestion and segmentation diagnostics. Useful for later reverse-proof / geometry reconstruction work.
- `dandi_000115`: event-aligned biological proxy bridge. Patch 15A probes NWB behavior layers; Patch 15B exports filtered arm-choice event traces accepted by the frozen VTE wrapper. DANDI 000115 is not treated as an explicit VTE-label benchmark.
