"""
Stage 3.1B: k_pos sweep runner.

Запускает:
1. balanced_conflict control
2. one-shot treat protocol

для нескольких значений k_pos при фиксированном w_qpos_input
...
def main():
    parser = argparse.ArgumentParser(description="Run Stage 3.1B k_pos sweep")
    parser.add_argument(
        "--values",
        nargs="+",
        type=float,
        required=True,
        help="Sweep values, e.g. 0.7 0.9 1.1 1.3 1.5"
    )
    parser.add_argument("--w-qpos-input-fixed", type=float, default=0.8)
    ...
    for k_pos in args.values:
        w_tag = fmt_tag(args.w_qpos_input_fixed)
        k_tag = fmt_tag(k_pos)

        param_suffix = f"_wqpi_{w_tag}_kpos_{k_tag}"

        row = {
            "w_qpos_input_fixed": float(args.w_qpos_input_fixed),
            "k_pos": float(k_pos),
            "balanced_run_dir": "",
            "one_shot_run_dir": "",
        }
        ...
        if args.include_balanced:
            run_cmd([
                ...
                "--ablation", "full",
                "--w-qpos-input-override", str(args.w_qpos_input_fixed),
                "--k-pos-override", str(k_pos),
            ])
    ...
    manifest_json = manifest_dir / f"k_pos_sweep_{args.one_shot_kind}.json"
