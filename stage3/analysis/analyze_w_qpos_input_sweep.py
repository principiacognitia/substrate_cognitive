"""
Stage 3.1B: w_qpos_input sweep analysis.

Читает manifest из run_w_qpos_input_sweep.py и строит:
- summary table
...
def build_table(manifest_rows):
    ...
    for spec in manifest_rows:
        row = {
            "w_qpos_input": float(spec["w_qpos_input"]),
            "k_pos_fixed": float(spec["k_pos_fixed"]),
        }
    ...
    return pd.DataFrame(rows).sort_values("w_qpos_input").reset_index(drop=True)
...
def main():
    parser = argparse.ArgumentParser(description="Analyze Stage 3.1B w_qpos_input sweep")
    ...
    out_csv = out_dir / "Table_3_1B_w_qpos_input_Sweep.csv"
    df.to_csv(out_csv, index=False)
    print(f"Saved: {out_csv}")

    if "balanced_p_open" in df.columns:
        plot_series(
            df, "w_qpos_input", "balanced_p_open",
            out_dir / "Figure_3_1B_wqpi_balanced_p_open.png",
            "Stage 3.1B: balanced_conflict P(open) vs w_qpos_input",
            "P(open)"
        )
        plot_series(
            df, "w_qpos_input", "balanced_p_timeout",
            out_dir / "Figure_3_1B_wqpi_balanced_timeout.png",
            "Stage 3.1B: balanced_conflict timeout vs w_qpos_input",
            "P(timeout)"
        )

    if "delta_post_minus_pre" in df.columns:
        plot_series(
            df, "w_qpos_input", "delta_post_minus_pre",
            out_dir / "Figure_3_1B_wqpi_one_shot_delta_post_minus_pre.png",
            "Stage 3.1B: one-shot treat carryover vs w_qpos_input",
            "Post minus pre P(open)"
        )
        plot_series(
            df, "w_qpos_input", "post_p_open",
            out_dir / "Figure_3_1B_wqpi_one_shot_post_p_open.png",
            "Stage 3.1B: one-shot treat post P(open) vs w_qpos_input",
            "Post P(open)"
        )
        plot_series(
            df, "w_qpos_input", "post_timeout",
            out_dir / "Figure_3_1B_wqpi_one_shot_post_timeout.png",
            "Stage 3.1B: one-shot treat post timeout vs w_qpos_input",
            "Post P(timeout)"
        )
