"""
Stage 3.1B: w_qneg_input sweep runner.

Запускает:
1. balanced_conflict control
2. one-shot shock protocol

для нескольких значений w_qneg_input и сохраняет manifest.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run Stage 3.1B w_qneg_input sweep")
    parser.add_argument(
        "--values",
        nargs="+",
        type=float,
    ...
    parser.add_argument("--one-shot-kind", type=str, default="shock", choices=["shock", "treat"])
    ...
    parser.add_argument("--one-shot-reward", type=float, default=None)
    parser.add_argument("--one-shot-salience", type=float, default=None)
    parser.add_argument("--one-shot-stakes", type=float, default=None)
    ...
    for w in args.values:
        ...
        if args.include_balanced:
            run_cmd([
                ...
                "--ablation", "full",
                "--w-qneg-input-override", str(w),
            ])
            row["balanced_run_dir"] = str(
                newest_run(base_dir, f"balanced_conflict_full_wqni_{tag}")
            )
        ...
        if args.include_one_shot:
            ...
            if args.one_shot_reward is not None:
                cmd.extend(["--one-shot-reward-override", str(args.one_shot_reward)])
            if args.one_shot_salience is not None:
                cmd.extend(["--one-shot-salience-override", str(args.one_shot_salience)])
            if args.one_shot_stakes is not None:
                cmd.extend(["--one-shot-stakes-override", str(args.one_shot_stakes)])

            run_cmd(cmd)

            forced_suffix = "_forced" if args.force_one_shot else ""
            row["one_shot_run_dir"] = str(
                newest_run(base_dir, f"one_shot_{args.one_shot_kind}_full{forced_suffix}_wqni_{tag}")
            )
    ...
    manifest_json = manifest_dir / f"w_qneg_input_sweep_{args.one_shot_kind}.json"
