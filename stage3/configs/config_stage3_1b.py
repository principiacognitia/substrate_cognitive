...
def get_canonical_conditions() -> Dict[str, Dict[str, Any]]:
    """
    Возвращает три канонические условия для абляций.
    
    Returns:
        canonical: Dict[name, condition_id]
    """
    return {
        'reward_dominant': {
            'condition_id': 'R2_T1',
            'description': 'High reward, low threat → P(open) ↑, deliberation moderate',
            'open_reward_prob': 0.90,
            'open_X_risk': 0.45
        },
        'balanced_conflict': {
            'condition_id': 'R1_T2',
            'description': 'Moderate reward, balanced threat → deliberation MAX',
            'open_reward_prob': 0.80,
            'open_X_risk': 0.60
        },
        'threat_dominant': {
            'condition_id': 'R0_T3',
            'description': 'No reward premium, high threat → P(covered) ↑, deliberation moderate/low',
...
def get_one_shot_protocol() -> Dict[str, Any]:
    ...
    Returns:
        protocol: Dict с pre/shock/post блоками
    """
    return {
        "pre_block_trials": 30,
        "shock_trial": 30,
...
def compute_conflict_variables(open_path: Dict, covered_path: Dict) -> Dict[str, Any]:
    ...
    Args:
        open_path: Конфиг открытого пути
        covered_path: Конфиг покрытого пути
    
    Returns:
        conflict_vars: Dict с reward_gap, risk_gap, threat_gap, conflict_index
    """
    # Expected rewards
    E_R_open = open_path['base_reward'] + open_path.get('reward_bonus', 0.0)
    E_R_open *= open_path['reward_prob']
    
    E_R_covered = covered_path['base_reward'] + covered_path.get('reward_bonus', 0.0)
    E_R_covered *= covered_path['reward_prob']
    ...
    # Risk values (X_risk from exposure profile)
    X_risk_open = open_path['exposure_profile']['X_risk']
    X_risk_covered = covered_path['exposure_profile']['X_risk']
    ...
    # Conflict index (simple absolute difference for now)
    # In full implementation, would use z-scores
    conflict_index = abs(reward_gap) - abs(threat_gap)
    
    # Determine which path is preferred by each factor
...
def build_env_config_for_condition(condition_id: str) -> Dict[str, Any]:
    """
    Строит полный env config для конкретного условия.
    
    Args:
        condition_id: ID условия (например, 'R1_T2')
    
    Returns:
        env_config: Полный конфиг среды
    """
