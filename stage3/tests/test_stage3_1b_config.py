...
class TestConflictVariables:
    ...
    def test_conflict_condition_classification(self):
        """Test that compute_conflict_variables returns numeric gaps and preferences only."""
        open_rd = {
        ...
        cv = compute_conflict_variables(open_rd, covered)

        assert 'reward_gap' in cv
        assert 'risk_gap' in cv
        assert 'threat_gap' in cv
        assert 'preferred_by_reward' in cv
        assert 'preferred_by_threat' in cv

        assert cv['preferred_by_reward'] in ['open', 'covered']
        assert cv['preferred_by_threat'] in ['open', 'covered']

        assert 'conflict_condition' not in cv
...
class TestBuildEnvConfig:
    ...
    def test_build_valid_condition(self):
        ...
        assert config['one_shot']['one_shot_enabled'] is False
        assert config['conflict_vars']['conflict_condition'] == 'balanced_conflict'
...
class TestConflictSemanticLabels:
    """Tests for semantic labels of canonical conditions."""

    def test_classify_reward_dominant(self):
        assert classify_conflict_condition('R2_T1') == 'reward_dominant'

    def test_classify_balanced_conflict(self):
        assert classify_conflict_condition('R1_T2') == 'balanced_conflict'

    def test_classify_threat_dominant(self):
        assert classify_conflict_condition('R0_T3') == 'threat_dominant'

    def test_classify_noncanonical_mixed(self):
        assert classify_conflict_condition('R2_T2') == 'mixed'
        assert classify_conflict_condition('R1_T1') == 'mixed'
...
class TestOneShotProtocol:
    ...
    def test_one_shot_config_defaults(self):
        ...
        assert os_config['one_shot_enabled'] is True
    ...
    def test_one_shot_disabled_default(self):
        """Regular condition builds must have one-shot OFF by default."""
        assert ONE_SHOT_DISABLED['one_shot_enabled'] is False
